  ...
  const links = user ? (
    <>
      ...
      {(user.role === 'admin' || user.role === 'sales') && (
        <NavLink className={linkClass} to="/products/new">
          <PackagePlus size={17} />
          Add Product
        </NavLink>
      )}
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="container-page flex h-16 items-center justify-between">
        <Link
          to="/"
          className="flex items-center gap-2 text-xl font-bold text-brand-900"
        >
        ...
        <div className="flex items-center gap-2">
          {user && (
            <Link
              to="/cart"
              className="relative rounded-lg p-2.5 hover:bg-slate-100"
            >
      ...
      {open && (
        <nav
          ...
          className="container-page grid gap-1 border-t py-3 lg:hidden"
        >
          <NavLink className={linkClass} to="/">
            Shop
          </NavLink>
